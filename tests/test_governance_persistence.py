...
throwaway home (``IDENTITY_AIOPS_HOME``) and assert that the rows compliance
...
def gov_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("IDENTITY_AUDIT_APPROVED_BY", raising=False)
...
def test_high_risk_allowed_with_named_approver(gov_home, monkeypatch):
    monkeypatch.setenv("IDENTITY_AUDIT_APPROVED_BY", "netops-alice")
...
def test_real_write_tool_persists_priorstate_undo(gov_home, monkeypatch):
    """End-to-end through a REAL product write tool: disable_user must capture
    the user's prior enabled state before mutating and persist the
    restore-undo (enable_user) on disk."""
    ...
    result = gov.disable_user(user_id="u1")
    assert result["priorState"] == {"enabled": True}
    assert result.get("_undo_id")

    undo = _rows(gov_home / "undo.db", "undo_log")
    assert len(undo) == 1
    assert undo[0]["undo_tool"] == "enable_user"
    assert json.loads(undo[0]["undo_params"]) == {"user_id": "u1"}

    audit = _rows(gov_home / "audit.db", "audit_log")
    assert [r["tool"] for r in audit] == ["disable_user"]
