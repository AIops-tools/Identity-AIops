...
carrying its own risk tier on the audit row). ``undo_apply`` is itself governed;
the inner tool it calls is audited in its own right.
...
def undo_apply(undo_id: str, dry_run: bool = False, target: Optional[str] = None) -> dict:
    ...
    The inverse runs through its own governed tool, so it is audited and carries
    its own risk tier there. Pass dry_run=True to preview the inverse call
