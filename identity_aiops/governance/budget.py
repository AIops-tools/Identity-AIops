...
2. **Runaway breaker (on by default).** The same ``(tool, params)`` called more
   than ``IDENTITY_RUNAWAY_MAX`` times within ``IDENTITY_RUNAWAY_WINDOW_SEC`` trips a
...
from identity_aiops.governance.policy import PolicyResult
...
class BudgetExceeded(Exception):
    ...
    Subclasses nothing identity-specific; the decorator wraps it into a
