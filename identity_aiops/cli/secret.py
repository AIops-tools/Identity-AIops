"""``identity-aiops secret`` — manage the encrypted credential store.

Secrets (IdP credentials) are stored in ``~/.identity-aiops/secrets.enc``
...
from identity_aiops.config import SECRET_ENV_PREFIX, SECRET_ENV_SUFFIX
from identity_aiops.secretstore import (
    SECRETS_FILE,
    SecretStore,
    check_permissions,
    migrate_legacy_env,
...
def secret_list() -> None:
    ...
    if not names:
        console.print(
            "[yellow]No secrets stored yet. Add one: identity-aiops secret set <name>[/]"
        )
        return
...
def secret_rotate_password() -> None:
    ...
    console.print("[green]✓ Master password rotated. Update IDENTITY_AIOPS_MASTER_PASSWORD.[/]")
